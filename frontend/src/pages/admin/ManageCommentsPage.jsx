...
const ManageCommentsPage = () => {
  ...
  const [replyDialog, setReplyDialog] = useState(null); // comment object
  ...
  return (
    ...
    >
      <PageHeader
        title="مدیریت نظرات"
        description="مدیریت و بررسی نظرات کاربران برای تمام موترها"
      />
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0 text-right">
                      ...
                      {comment.parent && (
                        <p className="text-xs text-zinc-400 mt-1">
                          در پاسخ به: "{comment.parent?.content?.slice(0, 60)}
                          ..."
                        </p>
                      )}
                    ...
                    <div className="flex flex-col gap-2 shrink-0">
                      ...
                      {comment.status === "approved" && (
                        ...
                        >
                          <Reply className="w-3.5 h-3.5 ml-1" />
                          {comment.adminReply ? "ویرایش پاسخ" : "پاسخ"}
                        </Button>
                      )}
      ...
      <Dialog open={!!replyDialog} onOpenChange={() => setReplyDialog(null)}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="text-right">پاسخ به نظر</DialogTitle>
          </DialogHeader>
          {replyDialog && (
            <div className="space-y-4">
              <div className="p-3 bg-zinc-50 dark:bg-zinc-900 rounded-xl border text-sm text-zinc-600 dark:text-zinc-400 text-right">
      ...
      <Dialog open={!!viewDialog} onOpenChange={() => setViewDialog(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="text-right">مشاهده کامل نظر</DialogTitle>
          </DialogHeader>
          {viewDialog && (
            <div className="space-y-3">
              ...
              <p className="text-sm text-zinc-700 dark:text-zinc-300 leading-relaxed text-right">
                {viewDialog.content}
              </p>
              {viewDialog.car && (
                <p className="text-xs text-zinc-400 text-right">
                  موتر: {viewDialog.car.name}
                </p>
              )}
              <p className="text-xs text-zinc-400 text-right">
                {new Date(viewDialog.createdAt).toLocaleString("fa-IR")}
              </p>
            </div>
          )}
        </DialogContent>
